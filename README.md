...
3. **계산 결과를 테스트로 고정** — pytest 104개와 GitHub Actions CI로 지표 계산과 표현 규칙이 깨지지 않는지 자동 검증합니다.
...
- **테스트**: pytest 104개 (GitHub Actions에서 자동 검증)
...
총 **104개** 테스트가 있습니다. (`main` 브랜치 기준, GitHub Actions에서 매 push마다 자동 실행)
