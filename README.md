...
<!-- 스크린샷 3장을 아래 경로에 추가한 뒤, 이 주석 블록의 시작/끝 표시만 지우면 이미지가 바로 표시됩니다.
     파일이 없는 상태로 이미지 태그를 켜두면 README에 깨진 이미지가 뜨므로 주석으로 두었습니다.

![전체 대시보드](docs/images/01_overview.png)

![채널별 성과 비교](docs/images/02_channel.png)
...
| 필요한 파일 | 화면 |
|---|---|
| `docs/images/01_overview.png` | 전체 대시보드 (KPI 카드 + 전기 대비 증감) |
| `docs/images/02_channel.png` | 채널별 성과 비교 |
| `docs/images/03_report.png` | 자동 요약 리포트 |

**스크린샷 이미지는 아직 미첨부 상태**입니다. 지금은 위의 라이브 데모 링크로 실제 화면을 확인해주세요.
...
- **제작 도구**: AI IDE(Kiro)를 활용해 코드를 구현했습니다.
- **직접 한 일**
  - 요구사항 정리와 KPI 정의 결정 (무엇을 어떤 식으로 계산할지)
  - 산출된 결과 검산 (화면 수치와 원본 데이터 대조)
  - 오류 발견 및 수정 지시 (예: 비용 지표 증감 색상이 반대로 표시되던 문제)
  - README 설명과 실제 코드·테스트 결과 대조
  - Streamlit Community Cloud 배포

※ 작성자 확인 필요 — 위 항목은 초안입니다. 실제 수행 범위에 맞게 수정해주세요.
