...
3. **계산 결과를 테스트로 고정** — pytest 94개와 GitHub Actions CI로 지표 계산과 표현 규칙이 깨지지 않는지 자동 검증합니다.
...
- **결과 검증** — 로컬에서 `pytest`와 `streamlit run`을 실행하고, 화면 KPI가 맞는지 `scripts/verify_kpi.py`로 **대시보드 코드를 쓰지 않고 원본 CSV에서 다시 계산해** 대조했습니다. ([검산 방법](#검산--대시보드-숫자를-다른-방법으로-다시-계산))
...
- **테스트**: pytest 94개 (GitHub Actions에서 자동 검증)
...
총 **94개** 테스트가 있습니다. (`main` 브랜치 기준, GitHub Actions에서 매 push마다 자동 실행)
