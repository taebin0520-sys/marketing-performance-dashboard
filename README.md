...
> 아래 경로에 이미지를 넣으면 자동으로 표시됩니다. (현재는 이미지 파일 미첨부 상태)
>
> - `docs/dashboard-overview.png` — 전체 대시보드 화면
> - `docs/kpi-period-comparison.png` — KPI 카드 + 전기 대비 증감
> - `docs/channel-performance.png` — 채널별 성과 비교
> - `docs/summary-report.png` — 자동 요약 리포트
...
| **전기 대비 증감** | 선택 기간과 같은 길이의 직전 기간을 자동 계산해 KPI 카드에 ▲▼ 증감률 표시. CPA·CPC는 감소가 개선이므로 색상 방향을 반대로 처리 |
...
| `test_report.py` | 16 | 리포트 6개 섹션 포함 여부, **결정론적 결과(같은 입력 → 같은 출력)**, 가장 크게 변한 지표 선정, **과도한 단정 표현 차단**, 조사 비문 방지 |
...
- 스크린샷 이미지 파일은 아직 첨부하지 않았습니다. README 상단에 들어갈 자리와 권장 파일명만 준비되어 있습니다.
